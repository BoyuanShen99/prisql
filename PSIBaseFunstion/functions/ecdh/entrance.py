...
async def data_prepare(read_path, store_path, inter, curve):
    # loca_data
    ...
    # 将sk, encrypted_data_t, local_data使用pickle序列化并保存到指定路径
    try:
        with open(store_path, 'wb') as f:
            pickle.dump({'sk': sk, 'encrypted_data': encrypted_data_bytes, 'local_data': local_data}, f)
...
async def process(store_path, save_path, inter, curve, local_port, remote_ip, remote_port):
    with open(store_path, 'rb') as f:
        data = pickle.load(f)

    sk = data['sk']
    encrypted_data_bytes = data['encrypted_data']
    ...
    logging.info("Step 1: exchange encrypted data")
    start = time.time()
    ...
    logging.info(f"Step 1 take {time.time() - start}")

    data_handler.received_data_event.clear()
    # final_encrypted_data
    logging.info("Step 2: final data process")
    start = time.time()
    ...
    logging.info("Step 3: exchange final data")
    ...
    # 本地求交
    # final_encrypted_data
    logging.info("Step 4: local intersection")
    start = time.time()
...
#                  args.read_path, args.temp_save, args.inter, curve))
