...
from functions.bc22.communication.OPRF.OPRFReceiver import OPRFReceiver
from functions.bc22.communication.handler import DataHandler, EndHandler, wait_for_remote_host
from functions.bc22.utils.cuckoo import CuckooHashTable
from functions.bc22.utils.funtions import unpack_byte_data, \
    pad_to_fixed_length, hash_to_finite_int, generate_large_prime
from functions.bc22.utils.io import read_csv, save_to_csv
...
class Bob(object):
    # Receiver
    def __init__(self, cuckoo, handler,
                 N, t, prime, r,
                 codewords=128):
...
def data_prepare(read_path, store_path, inter):
    ...
    try:
        with open(store_path, 'wb') as f:
            pickle.dump({'cuckoo': cuckoo}, f)

        logging.info(f"Data saved to {store_path}")
    except Exception as e:
        logging.error(f"Error saving data: {e}")


async def process(store_path, save_path, inter, local_port, remote_ip, remote_port):
    ...
    bob = Bob(cuckoo, data_handler, N=N, t=int(N * sparse_param), prime=prime, r=r)
    ...
    logging.info(f"prepare cost：{time.time() - start:.4f}s")
    ...
    logging.info(f"intersect cost：{time.time() - start:.4f}s")
    logging.info("finish intersection")

    res_strs = sorted(res)
    save_to_csv(res_strs, save_path, inter)

    logging.info(f"total program cost：{time.time() - start_time:.4f}s")
