...
from communication.VOLE.VOLEReceiver import VOLEReceiver
...
async def main(local_port, remote_ip, remote_port):
    ...
    r = 5
    lam = 256 // 8
    # beta 是t长度，模p（p=2）的数组
    # beta = np.array([random.randint(0, 1) for _ in range(2)])
    beta = np.array([1, 0])
    vole_receiver = VOLEReceiver(data_handler, x, beta=beta, N=16, n=20, t=2, prime=prime, r=r, lam=lam)

    await vole_receiver.gen()

    w = vole_receiver.expand()
